package com.rosouza.search.util;
...
public final class PaginationUtil {
    ...
    public static <T> Page<T> generatePage(HttpHeaders headers, List<T> contents) {
        long total = 0;
        if (headers != null && headers.getFirst(HEADER_X_TOTAL_COUNT) != null) {
            total = Long.parseLong(Objects.requireNonNull(headers.getFirst(HEADER_X_TOTAL_COUNT)));
        }
